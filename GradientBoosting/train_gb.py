# =========================
# train_model.py (with runtime tracking)
# =========================
import os
import time
import joblib
...
baseline_time = time.time() - start_time  # <-- end timing
...
results['baseline'] = {
   ...
   'runtime_sec': baseline_time  # <-- store runtime
}


baseline_model.save_model('models/xgb_baseline.json')
print("Baseline model saved at models/xgb_baseline.json")
...
manual_model.save_model('models/xgb_manual_tuned.json')
print("Manual tuned model saved at models/xgb_manual_tuned.json")
...
best_model.save_model('models/xgb_hypertuned.json')
print("Hypertuned model saved at models/xgb_hypertuned.json")
...
metrics_df.to_csv('models/metrics_comparison.csv')
print("All metrics saved at models/metrics_comparison.csv")
...
plt.tight_layout()
plt.savefig('models/feature_importance_manual_tuned.png')
plt.close()
...
plt.tight_layout()
plt.savefig('models/confusion_matrices_comparison.png')
plt.close()
...
plt.tight_layout()
plt.savefig('models/metrics_comparison_plot.png')
plt.close()
...
plt.grid(True)
plt.tight_layout()
plt.savefig('models/sensitivity_analysis.png')
plt.close()
...
print("\nAll plots saved in 'models/' folder.")
