...
model_path = 'GradientBoostingResults/xgb_hypertuned.json'
runtime_path = 'GradientBoostingResults/xgb_hypertuned_runtime.pkl'

if LOAD_SAVED_MODEL and os.path.exists(model_path):
    print("Loading saved hypertuned model...")
    best_model = xgb.XGBClassifier()
    best_model.load_model(model_path)

    if os.path.exists(runtime_path):
        hypertuned_time = joblib.load(runtime_path)
    else:
        hypertuned_time = None
    ...
    # ✅ save runtime
    joblib.dump(hypertuned_time, runtime_path)
...
for name in ['baseline', 'manual_tuned', 'hypertuned']:
    ...
    # ✅ Safe runtime handling
    if os.path.exists(runtime_path):
      hypertuned_time = joblib.load(runtime_path)
    else:
      print("⚠️ Runtime file missing — setting to 0.0")
      hypertuned_time = 0.0
